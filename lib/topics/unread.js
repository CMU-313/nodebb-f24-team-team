...
module.exports = function (Topics) {
	...
	async function filterTidsThatHaveBlockedPosts(params) {
		...
		const filteredTids = [];
		for (const tid of params.tids) {
			const hasUnblocked = await doesTidHaveUnblockedUnreadPosts(tid, {
				blockedUids: params.blockedUids,
				topicTimestamp: topicScores[tid],
				userLastReadTimestamp: userScores[tid],
			});
			if (hasUnblocked) {
				filteredTids.push(tid);
			}
		}
		return filteredTids;
	}
	...
	Topics.markUnread = async function (tid, uid) {
		...
		await Promise.all([
			db.sortedSetRemoveBulk([[`uid:${uid}:tids_read`, tid], [`tid:${tid}:bookmarks`, uid]]),
