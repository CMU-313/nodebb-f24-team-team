...
module.exports = function (Topics) {
	Topics.getSuggestedTopics = async function (tid, uid, start, stop, cutoff = 0) {
		...
		const [tagTids, searchTids] = await Promise.all(
			[getTidsWithSameTags(tid, tags.map(t => t.value), cutoff), getSearchTids(tid, title, cid, cutoff)]
		);
		...
		topicData = topicData.slice(start,
			stop !== -1 ? stop + 1 : undefined).sort((t1, t2) => t2.timestamp - t1.timestamp);
