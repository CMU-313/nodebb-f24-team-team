...
Flags.create = async function (type, id, uid, reason, timestamp, forceFlag = false) {
	...
	batched.push(db.setObject(`flag:${flagId}`, {
		...
		targetUid: targetUid,
		datetime: timestamp,
	}), Flags.addReport(flagId, type, id, uid, reason, timestamp), db.sortedSetAdd('flags:datetime', timestamp, flagId), db.sortedSetAdd(`flags:byType:${type}`, timestamp, flagId), db.sortedSetIncrBy('flags:byTarget', 1, [type, id].join(':')), analytics.increment('flags'));
	if (targetUid) {
