...
module.exports = function (Messaging) {
	...
	Messaging.removeUsersFromRoom = async (uid, uids, roomId) => {
		const [isOwner, userCount] = await Promise.all(
			[Messaging.isRoomOwner(uid, roomId), Messaging.getUserCountInRoom(roomId)]
		);
	...
	Messaging.loadRoom = async (uid, data) => {
		...
		const [room, inRoom, canChat, isAdmin, isGlobalMod] = await Promise.all(
			[Messaging.getRoomData(roomId), Messaging.isUserInRoom(uid, roomId),
				privileges.global.can(['chat', 'chat:privileged'], uid), user.isAdministrator(uid), user.isGlobalModerator(uid)]
		);
