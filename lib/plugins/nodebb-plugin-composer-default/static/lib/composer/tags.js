...
/* global define, ajaxify, config, app, socket, utils, $, window, jQuery */
...
define('composer/tags', ['alerts'], (alerts) => {
	...
	tags.init = function (postContainer, postData) {
		...
		app.loadJQueryUI(() => {
			input.autocomplete({
				...
				select: function () {
					// when autocomplete is selected from the dropdown simulate a enter key down
					triggerEnter(input);
				},
			});
	...
	function toggleTagInput(postContainer, postData, data) {
		...
		const hasPrivileges = data.privileges &&
            data.privileges.hasOwnProperty('topics:tag') &&
            !data.privileges['topics:tag'];
		const shouldHide = hasPrivileges ||
            (maxTags === 0 && !postData && !postData.tags && !postData.tags.length);

		postContainer.find('.tags-container')
			.toggleClass('haswhitelist', !!(data.tagWhitelist && data.tagWhitelist.length))
