...
define('composer/controls', ['composer/preview'], (preview) => {
	const controls = {};
	...
	controls.insertIntoTextarea = function (textarea, value) {
		...
		$textarea.val(currentVal.slice(0, payload.textarea.selectionStart) + payload.value + currentVal.slice(payload.textarea.selectionStart));
	...
	controls.replaceSelectionInTextareaWith = function (textarea, value) {
		...
		const currentVal = $textarea.val();
		const postContainer = $textarea.parents('[component="composer"]');
		$textarea.val(currentVal.slice(0, payload.textarea.selectionStart) + payload.value + currentVal.slice(payload.textarea.selectionEnd));
		preview.render(postContainer);
	};
	controls.wrapSelectionInTextareaWith = function (textarea, leading, trailing) {
		...
		$textarea.val(currentVal.slice(0, textarea.selectionStart) + matches[1] + leading + matches[2] + trailing + matches[3] + currentVal.slice(textarea.selectionEnd));
	...
	controls.getBlockData = function (textareaEl, query, selectionStart) {
		...
		payload = {
			in: !!(matchIndices.reduce((memo, cur) => {
				if (selectionStart >= cur + 2) {
					memo += 1;
				}
				return memo;
			}, 0) % 2),
			atEnd: matchIndices.reduce((memo, cur) => {
				if (memo) {
					return memo;
				}
