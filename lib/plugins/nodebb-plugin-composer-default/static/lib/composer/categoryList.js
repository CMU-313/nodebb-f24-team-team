...
/* global define, $, window, ajaxify, config */

define('composer/categoryList', ['categorySelector', 'taskbar', 'api'], (categorySelector, taskbar, api) => {
	const categoryList = {};
	let selector;
	categoryList.init = function (postContainer, postData) {
		...
		postContainer.on('action:composer.resize', () => {
		...
		postContainer.find('.category-name').translateHtml(selector.selectedCategory ? selector.selectedCategory.name : '[[modules:composer.select-category]]').on('click', () => {
	...
	categoryList.updateTaskbar = function (postContainer, postData) {
		if (parseInt(postData.cid, 10)) {
			api.get(`/categories/${postData.cid}`, {}).then((category) => {
	...
	async function changeCategory(postContainer, postData, selectedCategory) {
		...
		require(['composer/scheduler', 'composer/tags', 'composer/post-queue'], (scheduler, tags, postQueue) => {
