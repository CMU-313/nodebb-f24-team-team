...
define('composer/uploads', ['composer/preview', 'composer/categoryList', 'translator', 'alerts', 'uploadHelpers', 'jquery-form'], (preview, categoryList, translator, alerts, uploadHelpers) => {
	const uploads = {
		inProgress: {},
	};
	let uploadingText = '';
	...
	function onUploadError(xhr, post_uuid) {
		let msg = xhr.responseJSON && (xhr.responseJSON.error || xhr.responseJSON.status && xhr.responseJSON.status.message) || '[[error:parse-error]]';
