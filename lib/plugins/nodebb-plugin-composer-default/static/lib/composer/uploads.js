...
define('composer/uploads', ['composer/preview', 'composer/categoryList', 'translator', 'alerts', 'uploadHelpers', 'jquery-form'], function (preview, categoryList, translator, alerts, uploadHelpers) {
  var uploads = {
    inProgress: {}
  };
  var uploadingText = '';
  ...
  function addChangeHandlers(post_uuid) {
    ...
    postContainer.find('#files').on('change', function (e) {
      var files = (e.target || {}).files || ($(this).val() ? [{
        name: $(this).val(),
  ...
  function onUploadError(xhr, post_uuid) {
    var msg = xhr.responseJSON && (xhr.responseJSON.error || xhr.responseJSON.status && xhr.responseJSON.status.message) || '[[error:parse-error]]';
