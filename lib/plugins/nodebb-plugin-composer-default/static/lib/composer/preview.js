...
/* global define, socket, $, utils, localStorage, config */

define('composer/preview', ['hooks'], (hooks) => {
	const preview = {};
	preview.render = function (postContainer, callback) {
		...
		socket.emit('plugins.composer.renderPreview', textarea.val(), (err, preview) => {
			if (err) {
				return;
			}
	...
	preview.handleToggler = function ($postContainer) {
		...
		let show = !isMobile && (previewToggled === null && !hidePreviewOnOpen || previewToggled === 'true');
