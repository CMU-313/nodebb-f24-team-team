/* global define, $, window, localStorage, utils, ajaxify, config, app, socket, document, history */

'use strict';

define('composer', [
	'taskbar', 'translator', 'composer/uploads', 'composer/formatting',
	'composer/drafts', 'composer/tags', 'composer/categoryList', 'composer/preview',
	'composer/resize', 'composer/autocomplete', 'composer/scheduler',
	'composer/post-queue', 'scrollStop', 'topicThumbs', 'api', 'bootbox',
	'alerts', 'hooks', 'messages', 'search', 'screenfull',
], (
	taskbar, translator, uploads, formatting, drafts, tags,
	categoryList, preview, resize, autocomplete, scheduler, postQueue,
	scrollStop, topicThumbs, api, bootbox, alerts, hooks,
	messagesModule, search, screenfull
) => {
	const composer = {
		active: undefined,
		posts: {},
	...
	$(window).on('action:composer.topics.post', (ev, data) => {
	...
	$(window).on('popstate', () => {
		...
		if (composer.active && (env === 'xs' || env === 'sm')) {
			...
			translator.translate('[[modules:composer.discard]]', (translated) => {
				composer.discardConfirm = bootbox.confirm(translated, (confirm) => {
					if (confirm) {
						composer.discard(composer.active);
					} else {
	...
	function alreadyOpen(post) {
		const type = post.hasOwnProperty('cid') ? 'cid' :
			post.hasOwnProperty('tid') ? 'tid' :
				post.hasOwnProperty('pid') ? 'pid' : null;

		const id = post[type];

		// Find a match
		const matches = Object.keys(composer.posts).filter(uuid => (
			composer.posts[uuid].hasOwnProperty(type) && id === composer.posts[uuid][type]
		));
	...
	function push(post) {
		...
		translator.translate(actionText, (translatedAction) => {
	...
	async function composerAlert(post_uuid, message) {
		...
		const {
			showAlert,
		} = await hooks.fire('filter:composer.error', {
			post_uuid,
			message,
			showAlert: true,
		});
	...
	composer.newTopic = async (data) => {
		...
		({
			pushData,
		} = await hooks.fire('filter:composer.topic.push', {
			data: data,
			pushData: pushData,
		}));
		push(pushData);
	...
	composer.addQuote = function (data) {
		...
		const escapedTitle = (data.title || '').replace(/([\\`*_{}[\]()#+\-.!])/g, '\\$1').replace(/\[/g, '&#91;').replace(/\]/g, '&#93;').replace(/%/g, '&#37;')
			.replace(/,/g, '&#44;');
		if (data.body) {
			data.body = `> ${data.body.replace(/\n/g, '\n> ')}\n\n`;
		}
		const link = `[${escapedTitle}](${config.relative_path}/post/${encodeURIComponent(data.selectedPid || data.toPid)})`;
	...
	composer.newReply = function (data) {
		translator.translate(data.body, config.defaultLang, (translated) => {
			push({
				save_id: data.save_id,
				action: 'posts.reply',
				tid: data.tid,
	...
	composer.editPost = function (data) {
		// pid, text
		socket.emit('plugins.composer.push', data.pid, (err, postData) => {
			if (err) {
    ...
    */
		...
		postContainer.on('change', 'input, textarea', () => {
		...
		require(['mousetrap'], (mousetrap) => {
			mousetrap(postContainer.get(0)).bind('mod+enter', () => {
		...
		postContainer.find('.composer-minimize, .minimize .trigger').on('click', (e) => {
			e.preventDefault();
			e.stopPropagation();
			composer.minimize(post_uuid);
		});
		const textareaEl = postContainer.find('textarea');
		textareaEl.on('input propertychange', utils.debounce(() => {
			preview.render(postContainer);
		}, 250));
		textareaEl.on('scroll', () => {
	...
	async function createNewComposer(post_uuid) {
		...
		const title = postData.title.replace(/%/g, '&#37;').replace(/,/g, '&#44;');
		...
		let data = {
			...
			isTopic: isTopic,
			isEditing: isEditing,
			canSchedule: !!(isMain && privileges && (privileges['topics:schedule'] && !isEditing || isScheduled && privileges.view_scheduled)),
		...
		({
			postData,
			createData: data,
		} = await hooks.fire('filter:composer.create', {
			postData: postData,
			createData: data,
		}));
		app.parseAndTranslate('composer', data, (composerTemplate) => {
	...
	function mobileHistoryAppend() {
		...
		let returnPath = window.location.pathname.slice(1) + window.location.search;
	...
	async function post(post_uuid, anonymous = false) {
		...
		const isCategorySelected = !checkTitle || checkTitle && parseInt(postData.cid, 10);
		...
		api[method](route, composerData).then((data) => {
			...
			if (data.queued) {
				...
				if (onComposeRoute || composer.bsEnvironment === 'xs' || composer.bsEnvironment === 'sm') {
					window.history.back();
				} else if (submitHookData.redirect && (ajaxify.data.template.name !== 'topic' || ajaxify.data.template.topic && parseInt(postData.tid, 10) !== parseInt(ajaxify.data.tid, 10))) {
	...
	composer.updateThumbCount = function (uuid, postContainer) {
		...
		if (composerObj.action === 'topics.post' || composerObj.action === 'posts.edit' && composerObj.isMain) {
			...
			Promise.all(calls).then((thumbs) => {
				...
				formatEl.find('.badge').text(thumbCount).toggleClass('hidden', !thumbCount);
			});
		}
	};
	return composer;
