...
plugin.addPrefetchTags = async function (hookData) {
  const prefetch = ['/assets/src/modules/composer.js', '/assets/src/modules/composer/uploads.js', '/assets/src/modules/composer/drafts.js', '/assets/src/modules/composer/tags.js', '/assets/src/modules/composer/categoryList.js', '/assets/src/modules/composer/resize.js', '/assets/src/modules/composer/autocomplete.js', '/assets/templates/composer.tpl', `/assets/language/${meta.config.defaultLang || 'en-GB'}/topic.json`, `/assets/language/${meta.config.defaultLang || 'en-GB'}/modules.json`, `/assets/language/${meta.config.defaultLang || 'en-GB'}/tags.json`];
...
plugin.filterComposerBuild = async function (hookData) {
  ...
  const [isMainPost, postData, topicData, categoryData, isAdmin, isMod, formatting, tagWhitelist, globalPrivileges, canTagTopics, canScheduleTopics] = await Promise.all([posts.isMain(req.query.pid), getPostData(req), getTopicData(req), categories.getCategoryFields(req.query.cid, ['name', 'icon', 'color', 'bgColor', 'backgroundImage', 'imageClass', 'minTags', 'maxTags']), user.isAdministrator(req.uid), isModerator(req), plugin.getFormattingOptions(), getTagWhitelist(req.query, req.uid), privileges.global.get(req.uid), canTag(req), canSchedule(req)]);
...
async function getTagWhitelist(query, uid) {
  ...
  const [tagWhitelist, isAdminOrMod] = await Promise.all([categories.getTagWhitelist([cid]), privileges.categories.isAdminOrMod(cid, uid)]);
