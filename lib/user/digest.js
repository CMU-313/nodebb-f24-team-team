...
Digest.send = async function (data) {
	...
	await batch.processArray(data.subscribers, async (uids) => {
		...
		await Promise.all(userData.map(async (userObj) => {
			const [publicRooms, notifications, topics] = await Promise.all([getUnreadPublicRooms(userObj.uid),
				user.notifications.getUnreadInterval(userObj.uid, data.interval), getTermTopics(data.interval, userObj.uid)]);
...
async function getTermTopics(term, uid) {
	...
	const top = data.topics.filter(t => t.votes > 0 &&
		!popularTids.includes(t.tid)).sort((a, b) => b.votes - a.votes).slice(0, 10);
	const topTids = top.map(t => t.tid);
	const recent = data.topics.filter(t => !topTids.includes(t.tid) &&
	!popularTids.includes(t.tid)).sort((a, b) => b.lastposttime - a.lastposttime).slice(0, 10);
	[...top, ...popular, ...recent].forEach((topicObj) => {
		if (topicObj) {
			...
			const user = topicObj.hasOwnProperty('teaser') && topicObj.teaser &&
			topicObj.teaser.user ? topicObj.teaser.user : topicObj.user;
