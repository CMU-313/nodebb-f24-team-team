...
module.exports = function (User) {
	...
	User.hidePrivateData = async function (users, callerUID) {
		...
		const [userSettings, isAdmin, isGlobalModerator] = await Promise.all(
			[User.getMultipleUserSettings(users.map(user => user.uid)), User.isAdministrator(callerUID), User.isGlobalModerator(callerUID)]
		);
	...
	function parseDisplayName(user, uidToSettings) {
		...
		user.displayname = validator.escape(String(meta.config.showFullnameAsDisplayName &&
			showfullname && user.fullname ? user.fullname : user.username));
	}
