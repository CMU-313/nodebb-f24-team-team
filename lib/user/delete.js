...
module.exports = function (User) {
	...
	User.deleteAccount = async function (uid) {
		...
		await Promise.all([db.sortedSetRemoveBulk(bulkRemove), db.decrObjectField('global', 'userCount'), db.deleteAll(keys), db.setRemove('invitation:uids', uid), deleteUserIps(uid), deleteUserFromFollowers(uid), deleteUserFromFollowedTopics(uid), deleteUserFromIgnoredTopics(uid), deleteUserFromFollowedTags(uid), deleteImages(uid), groups.leaveAllGroups(uid), flags.resolveFlag('user', uid, uid), User.reset.cleanByUid(uid), User.email.expireValidation(uid)]);
		await db.deleteAll([`followers:${uid}`, `following:${uid}`, `user:${uid}`, `uid:${uid}:followed_tags`, `uid:${uid}:followed_tids`, `uid:${uid}:ignored_tids`]);
		delete deletesInProgress[uid];
		return userData;
	};
